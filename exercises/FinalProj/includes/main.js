...
function addRemoveFavorite(event){
    if ((event.target.className) == "fa fa-heart"){
        var recommendedLocation = document.getElementById("Recommended");
        event.target.className = "fa fa-heart-o";
        recommendedLocation.append(event.target.closest(".col-md-3"));
    
        var query = "UPDATE tbl_productsAVL_209 SET Type = 'Recommended' WHERE Title='"+this.id +"';";
        $.post('query.php', { query: query }, function (res) {//switch to query.php
            if(res == '[]'){
                console.log('error occured1 - username or password are invalid');
            }
        ...
        {
            var lastLocation = document.getElementById("Last Picks");
            event.target.className = "fa fa-heart";
            lastLocation.append(event.target.closest(".col-md-3"));

            var query = "UPDATE tbl_productsAVL_209 SET Type = 'Last Picks' WHERE Title='"+this.id +"';";
...
$(document).ready(function(){
    ...
    else if (url.includes("myUser")){
    //Last Picks
    var query = "SELECT Title,pic,Description,Type FROM tbl_productsAVL_209 WHERE Type='Last Picks';";
    addProducts_user(query, "fa fa-heart");
    //Recommended For You
    query = "SELECT Title,pic,Description,Type FROM tbl_productsAVL_209 WHERE Type='Recommended';";
    addProducts_user(query, "fa fa-heart-o"); 
    //What's New?
    query = "SELECT Title,pic,Description,Type FROM tbl_productsAVL_209 WHERE Type='Whats New';";
    addProducts_user(query, "fa fa-heart-o");   
    }
});
...
var addProducts_user = function (query, iconClass){
    $.post('query.php', { query: query }, function (res) { //switch to query.php
        ...
        if(res == '[]'){
            ...
            for(i=0;i<obj.length;++i){
                ...
                var linkIn1 = document.createElement("a");
                linkIn1.id = obj[i].Title;
                linkIn1.addEventListener("click",addRemoveFavorite);

                var i1 = document.createElement("i");
                i1.className = iconClass;
                ...
                var li2 = document.createElement("li");
                var linkIn2 = document.createElement("a");
                linkIn2.href = "./view_product.php?Title="+ obj[i].Title+"";
                
                var i2 = document.createElement("i");
                i2.className = "fa fa-eye";
                linkIn2.append(i2);
                li2.append(linkIn2);
                ul.append(li2);
                var newProduct = document.createElement("span");
